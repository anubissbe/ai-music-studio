...
import torch
import numpy as np
from audiocraft.models import MusicGen  # Zorg ervoor dat je MusicGen correct importeert
import os
import gc
...
def load_model_impl():
    ...
    try:
        ...
        model = MusicGen.get_pretrained('melody')  # Laad het juiste model
        model.set_generation_params(duration=30)  # Standaard voor 30 seconden
...
def generate_impl(content_prompt, output_path):
    ...
    try:
        ...
        audio_numpy = audio_output.cpu().numpy()[0, 0]  # Neem de eerste sample
